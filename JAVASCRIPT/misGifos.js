...
let pantallaMisGifosVacio = document.getElementById("gifos-vacios");
...
function buscarMisGifos() {
    ...
    if (misGifosString == null || misGifosString == "[]") {
        //Si no hay gif creados, se muestra pantalla con icono de vacio
...
function mostrarMisGifos(content) {
    ...
    for (let i = 0; i < gifosMisGifosArray.length; i++) {
        pantallaMisGifos.innerHTML += `<div class="resultados-gif-box-misgifos"('${content.data[i].images.downsized.url}', '${content.data[i].id}', '${content.data[i].slug}', '${content.data[i].username}', '${content.data[i].title}')">
